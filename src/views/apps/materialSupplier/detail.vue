            ...
            <div class="table_container">
              ...
              >
                ...
                <el-table-column fixed="right" label="操作" width="100">
                  <template slot-scope="scope">
                    ...
                    >保存</el-link>
                    <el-link type="primary" @click="materials_supplier_linkers_del(scope.row.id)">删除</el-link>
                  </template>
                </el-table-column>
              </el-table>
              ...
              <div style="text-align: center;margin-top: 30px">
                <el-button
                  v-if="formData.id"
                  type="primary"
                  @click="materials_supplier_put"
                >提交</el-button>
                <el-button v-else type="primary" @click="materials_suppliers_add">增加</el-button>
...
export default {
  ...
  methods: {
    materials_supplier_linkers_del(id) {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        api.materials_supplier_linkers_del(id).then(() => {
          this.getList();
        });
      });
    },
    ...
    adduser(row) {
      delete row.newid;
      api.materials_supplier_linkers_add({ ...row }).then(()=>{
         this.getList();
      });
    },
    ...
    materials_supplier_put() {
      console.log(this.formData, 5555);
      api.materials_supplier_put( { ...this.formData },  this.formData.id );
    },

    materials_suppliers_add() {
