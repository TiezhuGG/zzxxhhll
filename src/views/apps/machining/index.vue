<template>
  <div v-if="$route.name === 'machiningIndex'" class="customer_admin">
    <div class="container">
      ...
      <div class="flex-sp">
        <div class="content block">
          <div class="buttons flex-sp">
            <div>
              <el-button type="primary"  @click="$router.push({ path: 'machining_index/machining_detail'}) "  >
          ...
          <div class="table_container">
            <el-table ref="multipleTable" :data="tableData" :border="true" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" >
              ...
              <el-table-column fixed="right" label="操作" width="165">
                ...
                <el-link
                  type="primary"
                  @click="$router.push({ path: 'machining_index/machining_detail', query: scope.row} )" >编辑</el-link>
                <el-link type="primary" @click="manufacturers_del(scope.row.id)">删除</el-link>
...
export default {
  ...
  methods: {
    ...
    manufacturers_del(id){
         this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
              api.manufacturers_del(id)
              .then(()=>{
                this.getList();
              })
          })
